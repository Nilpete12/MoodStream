...
// 1. MOVED OUTSIDE: This prevents React from destroying and recreating the animation on every keystroke!
...
const Hero = () => {
  ...
  const handleMoodSearch = async (e) => {
    ...
    setIsAnalyzing(true);
    setTimeout(() => {
      navigate('/mood-results', { state: { prompt: moodPrompt } });
      ...
      <div className="relative z-20 w-full max-w-4xl px-6 text-center mt-12">
        ...
        >
          ...
          <form 
            ...
            className="relative flex items-center bg-black/70 backdrop-blur-xl border border-white/20 rounded-full p-2 shadow-2xl"
          >
            ...
            <input 
              ...
              className="w-full bg-transparent text-white placeholder-gray-400 px-4 py-4 outline-none font-cinematic text-sm md:text-lg disabled:opacity-50"
            />
            <button 
              type="submit"
              disabled={isAnalyzing || !moodPrompt.trim()}
              className="bg-white text-black px-8 py-3 rounded-full font-bold hover:bg-gray-200 transition-colors shrink-0 disabled:opacity-50 flex items-center gap-2"
            >
