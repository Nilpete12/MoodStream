...
const platforms = [
  ...
  { id: 'max', name: 'Max', bg: '#0047FF', text: 'text-white' },
  { id: 'apple', name: 'Apple TV+', bg: '#FFFFFF', text: 'text-black' },
  { id: 'crunchyroll', name: 'Crunchyroll', bg: '#F47521', text: 'text-white' }
];
...
const PlatformFilter = ({ activePlatform, setActivePlatform }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Trigger shift once page scrolls past 20px
  ...
  return (
    <div 
      className={`sticky z-40 w-fit mx-auto bg-vfxBlack/40 backdrop-blur-lg border rounded-xl border-white/5 px-4 py-2 transition-all duration-300 ease-in-out`}
      style={{ 
        transform: 'skewX(-20deg)',
        /* Top 80px (under navbar) initially, Top 20px when navbar fades */
        top: isScrolled ? '1.25rem' : '5rem' 
      }}
    >
